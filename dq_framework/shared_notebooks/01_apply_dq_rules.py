...
# MAGIC %md
# MAGIC # DQ Rule Application (shared)
# MAGIC
# MAGIC Parameterised notebook shared across all DQ entities.
# MAGIC Loads active rule mappings for the given target_table, applies each rule via
# MAGIC a dispatcher, and writes the enriched {target_table}_dq table.
# MAGIC
# MAGIC Two-column rules (RULE_40, RULE_41) use pipe-delimited field_name: "col_a|col_b".
# MAGIC Rows are NULL-filtered on status != 1 only when the source table has a status column.
...
for m in mappings:
    ...
    # Row-level status filter: only applies when source table has a status column
