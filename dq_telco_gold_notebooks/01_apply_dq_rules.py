...
# MAGIC # DQ Rule Application - telco_gold

# MAGIC

# MAGIC Loads all active rule mappings for telco_gold, applies each generic rule

# MAGIC dynamically via a dispatcher, and writes the enriched telco_gold_dq table

# MAGIC with a single DQ_RESULT column encoding all 10 rule outcomes.
...
# MAGIC telco_gold has no status column - all rows are evaluated.
...
# Load source table (copy already in DQ schema, _rescued_data already dropped)
...
# Build single DQ_RESULT column: "RULE_301: 1 | RULE_302: 1 | RULE_310: 0"
segments = []
for m in mappings:
    tmp_col = f"_DQ_{m.rule_mapping_id}"
