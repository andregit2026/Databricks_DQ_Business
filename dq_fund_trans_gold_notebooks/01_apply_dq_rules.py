...
# MAGIC # DQ Rule Application - fund_trans_gold

# MAGIC

# MAGIC Loads all active rule mappings for fund_trans_gold, applies each generic rule

# MAGIC dynamically via a dispatcher, and writes the enriched fund_trans_gold_dq table

# MAGIC with a single DQ_RESULT column encoding all 18 rule outcomes.

# MAGIC

# MAGIC DQ_RESULT format: "RULE_201: 1 | RULE_202: 0 | RULE_212: 0"
...
# MAGIC fund_trans_gold has no status column - all rows are evaluated.

# MAGIC Two-column rules (RULE_40, RULE_41) receive a pipe-delimited field_name: "col_a|col_b".
...
# Adds temporary column _DQ_{rule_no} with result: 1=pass, 0=fail
#
# Two-column rules pass field_name as "col_a|col_b" (pipe-delimited).
# RULE_40: field_3m|field_12m  - temporal window consistency
# RULE_41: count_field|amount_field - positive amount if count non-zero
...
def compute_rule_result(df, rule_no, generic_rule_desc_short, field_name):
    ...
    if generic_rule_desc_short == "RULE_1_DATE_NOT_NULL":
        ...
        # field_name = "field_3m|field_12m" - 3m window must not exceed 12m window
        f3m, f12m = field_name.split("|")
        return df.withColumn(tmp_col,
            F.when(
                F.col(f3m).isNotNull() & F.col(f12m).isNotNull() &
                (F.col(f3m) <= F.col(f12m)),
                F.lit(1)
            ).otherwise(F.lit(0)))

    elif generic_rule_desc_short == "RULE_41_POSITIVE_IF_COUNT_NONZERO":
        # field_name = "count_field|amount_field"
        # Pass if count == 0 (no transactions) OR amount > 0 (non-zero amount for non-zero count)
        count_f, amt_f = field_name.split("|")
        return df.withColumn(tmp_col,
            F.when(
                F.col(count_f).isNotNull() & F.col(amt_f).isNotNull() &
                ((F.col(count_f) == 0) | (F.col(amt_f) > 0)),
                F.lit(1)
            ).otherwise(F.lit(0)))
...
for m in mappings:
    ...
    # Row-level status filter: NULL for inactive rows (only if table has a status column)
...
# Build single DQ_RESULT column: "RULE_201: 1 | RULE_202: 0 | RULE_212: 0"
segments = []
for m in mappings:
    tmp_col = f"_DQ_{m.rule_mapping_id}"
